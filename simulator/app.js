...
ui.loginForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (!simState.organisations.length) {
      ...
      if (pendingEntries.length) {
        ui.loginStatus.textContent = `Authenticated (${pendingEntries.length} persisted case(s) pending cleanup, ${geocentredCount}/${simState.organisations.length} org geocenters, ${photoInventorySummary()})`;
      } else {
        ui.loginStatus.textContent = `Authenticated (${geocentredCount}/${simState.organisations.length} org geocenters, ${photoInventorySummary()})`;
      }
